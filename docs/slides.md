...
线上字节最高 **−99.8%** ｜ 吞吐最高 **+157%（2.6×）** ｜ p99 最高 **−67%**
...
| 基准 | ohmcp-bench | 七场景 + 扩展性扫描，每场景 3 次取中位数 |
...
**pipeline-64：吞吐 +138% ~ +157%（34.4 万 ops/s），p99 788µs → 360µs**
...
![七场景吞吐对比](benchmark-chart.svg)

七大场景吞吐全部为正 ｜ 59 项测试全绿 ｜ CI 零警告

---

## 工程质量与开源合规

- 42 单元 + 17 端到端集成测试（含对抗性：篡改/截断/超限帧/fuzz 不 panic），行覆盖率 90.0%；
