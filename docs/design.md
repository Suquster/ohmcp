...
同设备大 payload（默认 ≥16KB）可选绕过套接字数据拷贝，**上下行双向**：

- **协商**：客户端认证后发 `ShmSetup` 帧；服务端创建两块密封（SEAL_SHRINK/GROW/SEAL）
  的 memfd 环形缓冲区（下行环 + 上行环，各 4MiB），依次经 UDS 辅助数据
  （SCM_RIGHTS）传递两个 fd，再回 `ShmSetupResult`；双方 mmap 同一物理页；
- **数据面**：超阈值结果（服务端→客户端，下行环）与超阈值请求参数
  （客户端→服务端，上行环）均写入对应方向的环，帧内仅携带 12 字节引用
  （offset u64 + len u32，`SHM_REF` 标志位）；消费方按帧到达顺序（即写入顺序）
  取出并推进消费游标；
...
实测：下行（bulk-doc-64k）吞吐 +52%、p99 −67%、套接字字节 −99.8%；
上行（upload-256k，256KB 参数注入）吞吐 +81%、p50 −47%、套接字字节 −99.99%。
