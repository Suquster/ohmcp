...
|---|---|---|---|---|---|
| latency-echo | baseline | 40 774 | 24 | 27 | 1 000 869 |
| latency-echo | **ohmcp** | **49 240** | 19 | 25 | 963 272 |
| latency-echo | ohmcp（不加密） | 66 703 | 14 | 17 | 683 027 |
| bulk-kb-search | baseline | 29 716 | 31 | 36 | 16 035 879 |
| bulk-kb-search | **ohmcp** | **35 654** | 27 | 33 | 3 051 392 |
| bulk-kb-search | ohmcp（不加密） | 37 629 | 23 | 42 | 2 771 147 |
| bulk-doc-64k | baseline | 8 001 | 124 | 131 | 33 099 435 |
| bulk-doc-64k | **ohmcp** | **8 403** | 98 | 644 | 1 470 960 |
| repeat-cached | baseline | 30 335 | 32 | 38 | 15 718 089 |
| repeat-cached | **ohmcp** | **43 365** | 22 | 26 | 919 662 |
| pipeline-64 | baseline | 132 441 | 458 | 816 | — |
| pipeline-64 | **ohmcp** | **231 330** | 273 | 302 | — |
| concurrent-16 | baseline | 90 917 | 175 | 267 | — |
| concurrent-16 | **ohmcp** | **128 218** | 118 | 193 | — |
...
| bulk-doc-64k | **+5% ~ +20%** | −13% ~ −21% | **−95.6%** |
| pipeline-64 | **+66% ~ +84%** | −34% ~ −46% | — |
| concurrent-16 | **+30% ~ +41%** | −23% ~ −33% | — |

### 结论

- **通信效率**：大结果 −81%、热点重复调用 −94% 线上字节，远超赛题 ≥10% 要求；
- **时延/吞吐**：小消息、缓存命中、单连接多路复用场景全面领先（最高 +84% 吞吐、
  p99 尾延迟 776µs → 293µs）；
- **安全代价可量化**：全程 AEAD 加密下仍取得上述成绩；bulk 场景加密开销
  （每次 ~3KB 双向 AEAD）约抵消协议增益，为可按需关闭的合理折中；
- **大 payload 场景**：64KB 文档拉取吞吐 +5%、p50 −21%、线上字节 −95.6%——
  在软总线 / BLE / 低功耗蜂窝等带宽与能耗受限链路上，字节数直接决定时延与
  功耗，收益进一步放大；单次运行受压缩 CPU 抖动影响 p99 波动较大；
